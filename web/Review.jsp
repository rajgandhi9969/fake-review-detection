...
<head>
    ...
    <Script>
        ...
        function displayHelpfulReviews() {
            var newData = "<table class='table' style='width:100%'><thead class='thead-dark'><tr><th>Reviewer:</th><th>Comments:</th><th>Helpfulness:</th><th>Overall Rating</th><th>Sentiment Score</th><th>Check fake review</th></tr>";

            for (var i = 0; i < arguments.length; i++) {
                var data = arguments[i].split("#");
                var resultId="chkFake"+i;
                if(data[4]!=0)
                {
                    var helpful=((data[3] / data[4]) * 100).toFixed(2);
                }
                else
                {
                    var helpful=0;
                ...
                newData = newData + "<tr><td>" + data[0] + "</td><td style='width:28%'>" + data[1] + "</td><td>" + helpful + "%</td><td>" + data[2] + "</td><td>" + data[5] + "</td><td><div id=\""+resultId+"\"><button name=\""+resultId+"\" class='btn btn-primary' onclick='checkFake(\"" + data[6] + "\",\"" + data[1].replace("[(),]", "").replace(/'/g, "").replace("\"\"", "").replace(/"/g, "") + "\",\"" + data[7] + "\",\"" + data[2] + "\",this)'>Check Fake review</button></div></td></tr>";
            }
    ...
    <script>
         ...
         */
        function checkFake(reviewId, reviewText, reviewerID, overall,element) {

            var xhttp = new XMLHttpRequest();
            xhttp.onreadystatechange = function () {
                if (this.readyState == 4 && this.status == 200) {
                    document.getElementById(element.name).innerHTML=this.responseText;
                }
                if(this.status==400)
                {
                    document.getElementById(element.name).innerHTML="Unable to process, please try again later";
                }
            };
...
<%
    ...
    try {
        ...
        String connectionURL = "jdbc:mysql://localhost:3306/AmazonReviews:3306"; // AWS connection string
                            ...
                            <span><b>Helpfulness:</b></span><span>
                                <div class="progress">
                                    ...
                                    <%
                                        ...
                                        String helpful_reviews_sql = "select * from Reviews where asin='" + asin + "'  order by (helpfulness_assign/helpfulness_max) desc limit 25";
                                        ...
                                        while (resultSet3.next()) {
                                            String reviewText = resultSet3.getString("reviewText").replaceAll("[(),]", "").replaceAll("'", "\\\\'").replaceAll("\"\"", "").replaceAll("\n", " ");
